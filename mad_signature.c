...
int verifys(int * tim,element_t  sigs[][2],element_t * U,element_t * cs,int tag,int k){
	...
	for(int i=0;i<k;i++)
	{
        ...
        element_t sig1;
		element_init_G1(sig1,sigPairing);

		
		element_pow_zn(sig1,sigs[i][0],hash);
		element_mul(sig1,sig1,U[i]);
		element_mul(allSig[0],allSig[0],sig1);
...
void aggreaSig(element_t* ac,element_t  sigs[][2],element_t * cs ,int len){
	...
	element_init_G1(ac[0],sigPairing);
    element_init_G1(ac[1],sigPairing);
	
	for(int i=0;i<len;i++)
	{
        // verify(element_t*  sig,tim ,Us[i],cs[i],tag,i);
        element_t hash;
		element_init_Zr(hash,sigPairing);
		...
		element_init_G1(sig1,sigPairing);
		element_pow_zn(sig1,sigs[i][0],hash);
		element_mul(ac[0],sig1,ac[0]);
		element_mul(ac[1],sigs[i][1],ac[1]);
	}
